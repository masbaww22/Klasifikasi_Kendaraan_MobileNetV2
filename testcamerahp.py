...
IP_CAMERA_URL = "http://172.16.58.177:4747/video"
...
def capture_image_from_ip_camera(url):
    print(f"Mencoba terhubung ke kamera HP di {url}...")
    cap = cv2.VideoCapture(url)
    if not cap.isOpened():
        print(f"ERROR: Tidak dapat mengakses kamera HP di {url}!")
        print("Pastikan: \n1. Aplikasi DroidCam (atau serupa) berjalan di ponsel.\n2. Ponsel dan komputer berada di jaringan Wi-Fi yang sama.\n3. URL streaming benar (cek di aplikasi, misalnya /video atau /mjpegfeed).\n4. Port 4747 tidak diblokir oleh firewall.")
        return None
    
    print("Kamera HP aktif. Tekan 's' untuk mengambil gambar, 'q' untuk keluar.")
    while True:
        ret, frame = cap.read()
        if not ret:
            print("ERROR: Gagal menangkap gambar dari kamera HP!")
            print("Pastikan stream video aktif di aplikasi DroidCam.")
            cap.release()
            return None
        
        cv2.imshow('Kamera HP - Tekan "s" untuk simpan, "q" untuk keluar', frame)
        ...
        if key == ord('s'):
            ...
            image_path = os.path.join(temp_dir, f"ipcam_capture_{timestamp}.jpg")
...
print("\nSiapkan kamera HP untuk menangkap gambar kendaraan...")
print(f"   (Pastikan aplikasi DroidCam berjalan di {IP_CAMERA_URL})")
print("   (Tekan 's' untuk mengambil gambar, 'q' untuk keluar)")

image_path = capture_image_from_ip_camera(IP_CAMERA_URL)
